%\documentclass[line, margin]{res}
\documentclass[line, overlapped]{res}

\usepackage{enumitem} % Allows for adding options to lists
\setlist{nolistsep}

%\usepackage{paralist} % Alternative lists
%\pltopsep=-2pt
%\plitemsep=0pt
%\plparsep=0pt

\begin{document}
\name{Brandon Silver}
\address{1875 Arbor Valley Dr \\ Edmond, OK 73025 \\ (405) 598-7827 \\ brandon@brandonsilver.com}

\begin{resume}
    \section{OBJECTIVE}
    
    Seeking a full-time position as a network security analyst beginning June 2015.

    \section{EDUCATION}

    Oklahoma State University, Stillwater, OK \hfill Graduation: May 2015 \\
    \textbf{Bachelor of Science in Computer Science} \hfill GPA: 3.47
    \begin{itemize}
        \item Recipient of the General Honors Award
        \item Devon Energy Scholar, Spring 2014
    \end{itemize}


	\section{WORK EXPERIENCE} 

    \employer{Sandia National Laboratories}
    \title{Student Intern}
    \dates{May 2014 - August 2014}
    \location{Albuquerque, NM}
    \begin{position}
        \begin{itemize}
            \item Integrated a custom cloud back-end into a .NET front end
            \item Automated the management of Juniper SRX and SSG firewall devices
            \item Collaborated with a diverse selection of personnel
    \end{itemize}
    \end{position}

    \employer{USAA}
    \title{Software Developer Intern}
    \dates{May 2013 - August 2013}
    \location{San Antonio, TX}
    \begin{position}
        \begin{itemize}
            \item Analyzed and developed server-side web applications in a collaborative environment
            \item Produced code used by over 9 million users
            \item Coordinated with a third party vendor
    \end{itemize}
    \end{position} 

    \employer{Taylor Made Photography}
    \title{IT Technician, Photographer's Assistant}
    \dates{June 2012 - August 2012}
    \location{Edmond, OK}
    \begin{position}
        \begin{itemize}
            \item Maintained and upgraded over a dozen workstations and solved various technical issues
        \end{itemize}
    \end{position}

    \employer{Self Employed}
    \title{Webmaster}
    \dates{August 2008 -}
    \location{Edmond, OK}
    \begin{position}
        \begin{itemize}
            \item Install, configure, and maintain WordPress websites for small organizations.
        \end{itemize}
    \end{position}

	\section{TECHNICAL SKILLS}

	\begin{itemize}[leftmargin=10pt]
%    \begin{compactitem}
        \item Programming Languages:  Python, Ruby, Java, C\#, C/C++, LISP
        \item Databases: MySQL, SQLite, other SQL-based relational databases
        \item Network Analysis and Scanning Tools: nmap, Nessus
        \item Exploitation Tools: Metasploit, Armitage
        \item Network Firewalls: pfSense, Sophos UTM
        \item Operating Systems: Kali Linux, Ubuntu Linux, CentOS Linux, FreeBSD, Mac OS X, Windows(XP,Vista,7,8, Server 2008 R2)
        \item Configuration and use of Email, HTTP, SSH, (Open)VPN, ActiveDirectory Servers
%    \end{compactitem}
	\end{itemize}

%	\section{OTHER WORK EXPERIENCE}

%    \employer{Al's Cafe and Grill}
%    \title{Waiter}
%    \dates{Summer 2011, Summer 2012}
%    \location{Edmond, OK}
%    \begin{position}
%        \begin{itemize}
%            \item Waited tables, operated cash register, and cleaned dishes
%            \item Gained valuable insight into customer service
%        \end{itemize}
%    \end{position}

%    \employer{JMA Energy Company}
%    \title{Geology Technician Intern}
%    \dates{June 2009 - August 2009}
%    \location{Oklahoma City, OK}
%    \begin{position}
%        \begin{itemize}
%            \item Assisted the Geology Technician with the digitization and organization of hundreds of well logs.
%        \end{itemize}
%    \end{position}


    \section{PROJECTS}

    \begin{itemize}[leftmargin=10pt]
        \item \textbf{Student Grading System (March 2014 - April 2014)} Manages students' grades and enrollment, and features multiple user roles. Developed on a LAMP stack.
        \item \textbf{Startup Project Pinecone (May 2012 - January 2013)} An attempt at producing a crowd-sourced documentary and independent film streaming service built on Ruby on Rails.
        \item \textbf{Personal Website / Blog (2005 -)} My personal website has gone through several different iterations over the years, progressing between static HTML+CSS, WordPress, Drupal, and Jekyll implementations.
    \end{itemize}

\end{resume}
\end{document}
